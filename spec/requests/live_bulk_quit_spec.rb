...
RSpec.describe "WCA Live API" do
  describe "DELETE #bulk_quit_competitors" do
    ...
    it "Correctly quits a user from a first round" do
      ...
      delete api_v1_competition_live_bulk_quit_competitors_from_round_path(competition.id, round.wcif_id),
             params: { registration_ids: [registration_1.id] }
    ...
    it "Correctly quits a result from the first round and advances the next competitor" do
      ...
      delete api_v1_competition_live_bulk_quit_competitors_from_round_path(competition.id, final.wcif_id),
             params: live_request
      expect(response).to be_successful
    ...
    it "Correctly quits multiple competitors from a round in bulk" do
      sign_in delegate
      ...
      delete api_v1_competition_live_bulk_quit_competitors_from_round_path(competition.id, round.wcif_id),
             params: { registration_ids: quit_ids }

      expect(response).to be_successful

      quit_ids.each do |id|
        expect(LiveResult.find_by(round_id: round.id, registration_id: id)).to be_nil
      end
    end
  end
  ...
  describe "next_advancing_without with linked rounds" do
    ...
    it "excludes all linked-round results for multiple quitting competitors" do
      linked = create(:linked_round)
      round1 = create(:round, number: 1, total_number_of_rounds: 3, event_id: "333", competition: competition, linked_round: linked)
      round2 = create(:round, number: 2, total_number_of_rounds: 3, event_id: "333", competition: competition, participation_condition: ranking_condition, participation_source: linked, linked_round: linked)
      final = create(:round, number: 3, total_number_of_rounds: 3, event_id: "333", competition: competition, participation_condition: ranking_condition, participation_source: linked)
      ...
      # Ranking by best result across both rounds:
      #   registrations[0]: best = round2(100)  → rank 1 (advancing, quitting)
      #   registrations[1]: best = round2(200)  → rank 2 (advancing, quitting)
      #   registrations[2]: best = round2(300)  → rank 3 (advancing)
      #   registrations[3]: best = round2(400)  → rank 4 (next up after two quit)
      #   registrations[4]: best = round2(500)  → rank 5 (next up after two quit)
      5.times do |i|
        create(:live_result, registration: registrations[i], round: round1, average: 9000)
        create(:live_result, registration: registrations[i], round: round2, average: (i + 1) * 100)
      end
