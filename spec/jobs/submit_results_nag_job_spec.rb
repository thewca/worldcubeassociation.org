...
RSpec.describe SubmitResultsNagJob do
  it "schedules results nag email" do
    _unscheduled_competition = create(:competition, starts: nil)
    _recent_competition_missing_results = create(:competition, :visible, starts: 3.days.ago)
    old_competition_missing_results = create(:competition, :visible, starts: 3.weeks.ago)
    _older_competition_missing_results_but_already_nagged = create(:competition, :visible, starts: 3.weeks.ago, results_nag_sent_at: 1.day.ago)
    older_competition_missing_results_nagged_a_long_time_ago = create(:competition, :visible, starts: 3.weeks.ago, results_nag_sent_at: 8.days.ago)
