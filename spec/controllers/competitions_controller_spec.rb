...
RSpec.describe CompetitionsController do
  ...
  describe 'POST #update' do
    ...
    context "when signed in as a trainee delegate" do
      ...
      it "cannot change extra registration requirements field after competition is confirmed" do
        comp = create(:competition, :confirmed, delegates: [delegate, trainee_delegate], extra_registration_requirements: "Extra requirements")
        new_requirements = "New extra requirements"
        update_params = build_competition_update(competition, registration: { extraRequirements: new_requirements })
        patch :update, params: update_params, as: :json
        comp.reload
    ...
    context 'when signed in as results team member' do
      ...
      it "sends the notification emails to users that competed" do
        create_list(:user_with_wca_id, 4, results_notifications_enabled: true).each do |user|
          create(:result, person: user.person, competitionId: competition.id, eventId: "333")
        end
      ...
      it "assigns wca id when user matches one person in results" do
        ...
        create(:result, competition: competition, person: reg.person, eventId: "333")
      ...
      it "does not assign wca id when user matches several persons in results" do
        ...
        create(:result, competition: competition, person: person, eventId: "333")
        another_person = create(:person, name: person.name, countryId: person.countryId, gender: person.gender, dob: person.dob)
        create(:result, competition: competition, person: another_person, eventId: "333")
      ...
      it "does not assign wca id when user matches results but wca id is already assigned" do
        ...
        create(:registration, :accepted, competition: competition, user: user)
        create(:result, competition: competition, person: user.person, eventId: "333")
  ...
  describe 'GET #my_competitions', clean_db_with_truncation: true do
    let(:delegate) { create(:delegate) }
    let(:organizer) { create(:user) }
    let!(:future_competition1) { create(:competition, :registration_open, starts: 5.week.from_now, organizers: [organizer], delegates: [delegate], events: Event.where(id: %w(222 333))) }
    ...
    let!(:past_competition2) { create(:competition, starts: 2.month.ago, delegates: [delegate], events: Event.where(id: %w(222 333))) }
    let!(:past_competition3) { create(:competition, starts: 3.month.ago, delegates: [delegate], events: Event.where(id: %w(222 333))) }
    let!(:past_competition4) { create(:competition, :results_posted, starts: 4.month.ago, delegates: [delegate], events: Event.where(id: %w(222 333))) }
