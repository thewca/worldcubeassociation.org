...
RSpec.describe PersonsController, type: :controller do
  describe "GET #index" do
    ...
    context "Ajax request", clean_db_with_truncation: true do
      let!(:person1) { create(:person, name: "Jennifer Lawrence", countryId: "USA", wca_id: "2016LAWR01") }
      let!(:person2) { create(:person, name: "Benedict Cumberbatch", countryId: "United Kingdom", wca_id: "2016CUMB01") }
      let!(:competition) { create(:competition) }
      let!(:result) { create(:result, pos: 1, roundTypeId: "f", competitionId: competition.id, person: person1) }
