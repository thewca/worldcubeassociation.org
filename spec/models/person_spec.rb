...
RSpec.describe Person, type: :model do
  ...
  context "likely_delegates" do
    ...
    it "works" do
      ...
      create :result, person: person, competitionId: competition2.id
      expect(person.likely_delegates).to eq [delegate]

      new_delegate = create :delegate
      competition3 = create :competition, delegates: [new_delegate], starts: 2.days.ago
      create :result, person: person, competitionId: competition3.id
  ...
  describe "updating the data" do
    ...
    context "fixing the person" do
      it "fixing countryId fails if there exists an old person with the same wca id, greater subId and the same countryId" do
        create(:person, wca_id: person.wca_id, subId: 2, name: person.name, countryId: "New Zealand")
      ...
      it "doesn't update personName and countryId columns in the results table if they differ from the current ones" do
        create(:person_who_has_competed_once, wca_id: person.wca_id, subId: 2, name: "Old Name", countryId: "France")
        person.update!(name: "New Name", countryId: "New Zealand")
  ...
  describe "#world_championship_podiums" do
    ...
    let!(:result1) { create :result, person: person, competition: wc2015, pos: 2, eventId: "333" }
    let!(:result2) { create :result, person: person, competition: wc2015, pos: 1, eventId: "333oh" }
    let!(:result3) { create :result, person: person, competition: wc2017, pos: 3, eventId: "444" }
  ...
  describe "#championship_podiums" do
    ...
    let!(:fr_competitor) do
      create(:person, countryId: "France").tap do |fr_competitor|
        create :result, person: fr_competitor, competition: fr_nationals2016, pos: 1, eventId: "333"
        create :result, person: fr_competitor, competition: us_nationals2017, pos: 1, eventId: "333"
      end
    end
    let!(:us_competitor) do
      create(:person, countryId: "USA").tap do |us_competitor|
        create :result, person: us_competitor, competition: us_nationals2017, pos: 2, eventId: "333"
      end
    end
    ...
    context "when a person changed nationality and continent" do
      ...
      it "does no longer treat the person as eligible for championship title related to previous nationality" do
        expect do
          ...
          create :result, person: fr_competitor, competition: fr_nationals2017, pos: 1, eventId: "333"
      ...
      it "is eligible for championship title of the current continent" do
        expect do
          ...
          create :result, person: fr_competitor, competition: na_championship2017, pos: 1, eventId: "333"
    ...
    it "reassigns positions correctly in the case of a tie" do
      us_competitor1 = create :person, countryId: "USA"
      us_competitor2 = create :person, countryId: "USA"
      us_competitor3 = create :person, countryId: "USA"
      create :result, person: fr_competitor, competition: us_nationals2017, pos: 1, eventId: "222"
      create :result, person: us_competitor1, competition: us_nationals2017, pos: 2, eventId: "222"
      create :result, person: us_competitor2, competition: us_nationals2017, pos: 2, eventId: "222"
      create :result, person: us_competitor3, competition: us_nationals2017, pos: 4, eventId: "222"
