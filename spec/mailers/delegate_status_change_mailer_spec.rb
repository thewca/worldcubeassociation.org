...
RSpec.describe DelegateStatusChangeMailer, type: :mailer do
  describe 'notify_board_and_assistants_of_delegate_status_change' do
    ...
    it 'promoting a registered speedcuber to a delegate' do
      ...
      expect(mail.body.encoded).to match("#{senior_delegate_role1.user.name} has changed the Delegate status of Jon Snow from Registered Speedcuber to Junior Delegate.")
    ...
    it 'promoting a Junior delegate to a delegate' do
      ...
      expect(mail.body.encoded).to match("#{senior_delegate_role1.user.name} has changed the Delegate status of Daenerys Targaryen from Junior Delegate to Delegate.")
      expect(mail.body.encoded).not_to match('Warning')
    ...
    it 'demoting a Junior delegate to a registered speedcuber' do
      ...
      expect(mail.body.encoded).to match("#{senior_delegate_role1.user.name} has changed the Delegate status of Daenerys Targaryen from Junior Delegate to Registered Speedcuber.")
      expect(mail.body.encoded).not_to match('Warning')
      expect(mail.body.encoded).to match(edit_user_url(delegate))
    end
    ...
    it 'renders a warning if someone other than their senior delegate makes the change' do
      ...
      expect(mail.body.encoded).to match("Warning: #{senior_delegate_role2.user.name} is not Daenerys Targaryen's Senior Delegate.")
    end
  end
end
