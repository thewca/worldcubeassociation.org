...
import { Card, Text, Heading, Table, Center, Spinner} from "@chakra-ui/react"
...
const TabCompetitors: React.FC<CompetitorData> = ({id}) => {
    ...
    useEffect(() => {
        fetch("https://www.worldcubeassociation.org/api/v0/competitions/" + id + "/wcif/public")
          .then(res => res.json())
          .then(data => {
            ...
            // Get a unique list of event IDs across all competitors
            const allEventIds = new Set<string>();
            people.forEach(p => {
              p.registration?.eventIds.forEach(id => allEventIds.add(id));
            });
    
            setEventIds(Array.from(allEventIds).sort()); // optional: sort alphabetically
          })
          .catch(console.error)
        ...
        <Table.Body>
          {competitors.map((person) => (
            <Table.Row key={person.wcaId || person.name}>
              ...
              <Table.Cell><CountryMap code={person.countryIso2} bold/></Table.Cell>
