...
// "use client" is required here: Ark UI's `asChild` (used by Menu.Trigger, Collapsible.Trigger
// and Menu.Item below) renders nothing when its child element crosses the RSC boundary, because
// the child then arrives as a lazy client reference instead of a React element. Building this
// tree inside a client module keeps SSR and hydration in sync. Data fetching stays in navbar.tsx.
...
export default function NavbarContent({
  ...
  navbarEntries: Nav["entry"];
  socialLinks: NonNullable<SocialLink["links"]>;
  session: Session | null;
  liveResultBeta: boolean;
}) {
  const showEmptyMessage = !liveResultBeta && navbarEntries.length === 0;
