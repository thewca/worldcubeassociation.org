WITH competitions_with_result_advancement AS (
  SELECT DISTINCT
  ...
  WHERE REPLACE(REGEXP_SUBSTR(rounds.advancement_condition, '"(.*?)"', 9), '"', '') = 'attemptResult'
),
adv_cond AS (
  SELECT
    rounds.id,
  ...
  LEFT JOIN competition_events AS cevents
  ON rounds.competition_event_id = cevents.id
  WHERE (cevents.competition_id, cevents.event_id) IN (SELECT competition_id, event_id FROM competitions_with_result_advancement)
),
re AS (
  SELECT DISTINCT
...
ON re.competition_id = re1.competition_id
  ...
  AND re.person_id = re1.person_id
  AND re.number = re1.number - 1
WHERE re.result >= re.adv_count
  AND re.adv_type = 'attemptResult';
