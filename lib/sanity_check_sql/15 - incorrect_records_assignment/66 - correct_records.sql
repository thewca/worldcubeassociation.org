WITH round_dates AS (
  ...
  FROM schedule_activities AS sa
  JOIN venue_rooms AS vr
  ON sa.venue_room_id = vr.id
  JOIN competition_venues AS cv
...
-- Joins round date to results table and filters out rows that are not <= NRs from previous years.
-- Assigns ranking 1 to each single or average that is the best for that country for that day.
t1 AS (
  SELECT
    r.id AS results_id,
    ...
    rn.number AS round,
    RANK() OVER (
      PARTITION BY r.country_id, r.event_id, IF(rd.round_date IS NOT NULL, rd.round_date, c.start_date)
      ORDER BY CASE WHEN r.best > 0 THEN r.best ELSE 999999999999 END
    ) AS day_best_single,
    RANK() OVER (
      PARTITION BY r.country_id, r.event_id, IF(rd.round_date IS NOT NULL, rd.round_date, c.start_date)
      ORDER BY CASE WHEN r.average > 0 THEN r.average ELSE 999999999999 END
    ) AS day_best_average,
    r.best,
    r.average,
  ...
  JOIN rounds AS rn
  ON rn.id = r.round_id
  LEFT JOIN round_dates AS rd
  ON rd.round_id = r.round_id
  LEFT JOIN old_nr_singles AS ons
  ON ons.country_id = r.country_id
    AND ons.event_id = r.event_id
  LEFT JOIN old_nr_averages AS ona
...
-- Removes rows from t1 that are not the fastest result of that day.
-- Calculates whether or not each result is NR single or average.
t2 AS (
  SELECT
    t1.results_id,
    t1.round_date,
...
-- Joins t2 to continental and world records from previous year.
-- Calculates whether or not each result is CR or WR single/average.
t3 AS (
  SELECT
    c.continent_id,
    CASE c.continent_id
