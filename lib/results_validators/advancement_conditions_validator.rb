  ...
  class AdvancementConditionsValidator < GenericValidator
    ...
    def run_validation(validator_data)
      ...
      validator_data.each do |competition_data|
        ...
        number_of_rounds_per_event = competition.rounds.reorder(nil).group(:event_id).count
        previous_round = nil

        competition.rounds.each do |round|
          next if IGNORE_ROUND_TYPES.include?(round.round_type_id)
          ...
          if previous_round.present?
            previous_results = previous_round.results
            ...
            if condition.instance_of? AdvancementConditions::AttemptResultCondition
              current_persons = round.results.pluck(&:person_id)
