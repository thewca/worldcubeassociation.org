  ...
  class AdvancementConditionsValidator < GenericValidator
    ...
    def run_validation(validator_data)
      validator_data.each do |competition_data|
        ...
        results_by_round_id = competition_data.results.group_by(&:round_id)

        rounds_without_deprecated_types = competition.rounds.reject { IGNORE_ROUND_TYPES.include?(it.round_type_id) }
        previous_round = nil
