...
module ResultsValidators
  class PersonsValidator < GenericValidator
    ...
    def self.name_validations(name, competition_id = nil, **_message_args)
      ...
      # # Regex %r{[^\p{L}\p{M}\p{Zs}\p{Cf}\-'.''()·•/]} uses a negated character class —
      ...
      validation_issues << ValidationWarning.new(SPECIAL_CHARACTERS_IN_NAME_WARNING, :persons, competition_id, name: name) if %r{[^\p{L}\p{M}\p{Zs}\p{Cf}\-'.()·•/]}.match?(name)
