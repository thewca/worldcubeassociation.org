...
module Registrations
  class RegistrationChecker
    ...
    def self.update_registration_allowed!(update_request, competition, requester_user)
      ...
      raise WcaExceptions::RegistrationError.new(:not_found, ErrorCodes::REGISTRATION_NOT_FOUND)
    end
    ...
    class << self
      def user_can_create_registration!(competition, requester_user, requestee_user)
        ...
        raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) unless requester_user.id == requestee_user.id

        # Only organizers can register when registration is closed, and they can only register for themselves - not for other users
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::REGISTRATION_CLOSED) unless competition.registration_open? || organizer_modifying_own_registration?(competition, requester_user, requestee_user)

        # Users must have the necessary permissions to compete - eg, they cannot be banned or have incomplete profiles
        raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::USER_CANNOT_COMPETE) unless requestee_user.cannot_register_for_competition_reasons(competition).empty?

        # Users cannot sign up for multiple competitions in a series
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::ALREADY_REGISTERED_IN_SERIES) if existing_registration_in_series?(competition, requestee_user)
      end

      def user_can_modify_registration!(competition, requester_user, requestee_user)
        raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) unless can_administer_or_current_user?(competition, requester_user, requestee_user)
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::USER_EDITS_NOT_ALLOWED) unless competition.registration_edits_allowed? || requester_user.can_manage_competition(competition)
        raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::REGISTRATION_IS_REJECTED) if user_is_rejected?
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::ALREADY_REGISTERED_IN_SERIES) if
      ...
      def validate_create_events!(request, competition)
        ...
        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::INVALID_EVENT_SELECTION) unless competition.events_held?(event_ids)

        event_limit = competition.events_per_registration_limit
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::INVALID_EVENT_SELECTION) if event_limit.present? && event_ids.count > event_limit
      end
      ...
      def validate_qualifications!(request, competition)
        ...
        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::QUALIFICATION_NOT_MET, unqualified_events) unless unqualified_events.empty?
      end

      def validate_guests!(request, competition)
        return if (guests = request['guests'].to_i).nil?

        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::INVALID_REQUEST_DATA) unless Registration.build(competition_id: competition, guests: guests).guests_valid?
      end
      ...
      def validate_comment!(request, competition, registration = nil)
        if (comment = request.dig('competing', 'comment')).nil?
          ...
          raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::REQUIRED_COMMENT_MISSING) if competition.force_comment_in_registration
        else
          raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::INVALID_REQUEST_DATA) unless Registration.build(competition_id: competition, comments: comment).comments_valid?
        end
      end
      ...
      def validate_organizer_fields!(request)
        ...
        raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) if contains_organizer_fields?(request, organizer_fields) && !requester_user.can_manage_competition(competition)
      end

      def validate_organizer_comment!(request)
        organizer_comment = request.dig('competing', 'organizer_comment')
        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::USER_COMMENT_TOO_LONG) if
      ...
      def validate_waiting_list_position!(request, competition)
        ...
        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::INVALID_WAITING_LIST_POSITION) if waiting_list_position.is_a? Float
        ...
        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::INVALID_WAITING_LIST_POSITION) unless converted_position.is_a? Integer

        waiting_list = competition.waiting_list.entries
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::INVALID_WAITING_LIST_POSITION) if waiting_list.empty? && converted_position != 1
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::INVALID_WAITING_LIST_POSITION) if converted_position > waiting_list.length
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::INVALID_WAITING_LIST_POSITION) if converted_position < 1
      end
      ...
      def validate_update_status!(request, competition, requester_user, requestee_user, registration)
        ...
        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::INVALID_REQUEST_DATA) unless RegistrationHelper::REGISTRATION_STATES.include?(new_status)
        raise WcaExceptions::RegistrationError.new(:forbidden, ErrorCodes::COMPETITOR_LIMIT_REACHED) if
        ...
        if new_status == 'pending'
          raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) unless registration.cancelled?
        ...
        raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) if new_status != 'cancelled'

        # Raise an error if competition prevents users from cancelling a registration once it is accepted
        raise WcaExceptions::RegistrationError.new(:unauthorized, ErrorCodes::ORGANIZER_MUST_CANCEL_REGISTRATION) if
          !competition.user_can_cancel? && registration.accepted?

        # Users aren't allowed to change events when cancelling
        raise WcaExceptions::RegistrationError.new(:unprocessable_entity, ErrorCodes::INVALID_REQUEST_DATA) if
