  ...
  class RegistrationChecker
    ...
    class << self
      def user_can_create_registration!(competition, requester_user, requestee_user)
        ...
        raise RegistrationError.new(:forbidden, ErrorCodes::ALREADY_REGISTERED_IN_SERIES) if existing_registration_in_series?(competition, requestee_user)
      end

      def user_can_modify_registration!(competition, requester_user, requestee_user)
        raise RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) unless can_administer_or_current_user?(competition, requester_user, requestee_user)
        raise RegistrationError.new(:forbidden, ErrorCodes::USER_EDITS_NOT_ALLOWED) unless competition.registration_edits_allowed? || requester_user.can_manage_competition(competition)
      ...
      def validate_guests!(request, competition)
        ...
        raise RegistrationError.new(:unprocessable_entity, ErrorCodes::GUEST_LIMIT_EXCEEDED) if competition.guest_limit_exceeded?(guests)
      end
      ...
      def validate_comment!(request, competition, registration = nil)
        if (comment = request.dig('competing', 'comment')).nil?
          ...
          raise RegistrationError.new(:unprocessable_entity, ErrorCodes::REQUIRED_COMMENT_MISSING) if competition.force_comment_in_registration
        else
          ...
          raise RegistrationError.new(:unprocessable_entity, ErrorCodes::REQUIRED_COMMENT_MISSING) if competition.force_comment_in_registration && comment == ''
        end
      end
      ...
      def validate_organizer_fields!(request)
        ...
        raise RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) if contains_organizer_fields?(request, organizer_fields) && !requester_user.can_manage_competition(competition)
      end
      ...
      def validate_update_status!(request, competition, requester_user, requestee_user, registration)
        ...
        if new_status == 'pending'
          raise RegistrationError.new(:unauthorized, ErrorCodes::USER_INSUFFICIENT_PERMISSIONS) unless registration.cancelled?
