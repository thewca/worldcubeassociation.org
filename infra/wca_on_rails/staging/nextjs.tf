...
locals {
  nextjs_environment = [
    ...
    {
      name  = "OIDC_ISSUER"
      value = "https://www.staging.worldcubeassociation.org/"
    },
    {
      name  = "DATABASE_URI"
    ...
    },
    {
      ...
      value = "http://${local.rails_internal_dns}:3000/api/"
    },
    {
      name  = "WCA_FRONTEND_API_URL"
      value = "https://www.staging.worldcubeassociation.org/api/"
    },
    {
...
resource "aws_ecs_task_definition" "nextjs" {
  ...
  container_definitions = jsonencode([
    {
      name              = "nextjs-staging"
      image             = "${var.shared.next_repository_url}:nextjs-staging"
      cpu    = 1024
      memory = 3910
      portMappings = [
        {
...
resource "aws_ecs_service" "nextjs" {
  ...
  load_balancer {
    target_group_arn = var.shared.nextjs_staging.arn
