...
module RegulationTranslationsHelper
  ...
  private
    ...
    def with_relative_url(hash, tag)
      hash[:url] = "./#{tag}"
      hash
    end

    def regulations_bucket
      Aws::S3::Resource.new(
        credentials: Aws::ECSCredentials.new,
      ).bucket(BUCKET_NAME)
    end

    def current_build_hash(s3_client = regulations_bucket)
      s3_client.object(TRANSLATIONS_HASH_FILE).get.body.read.strip
    end

    def current_base_version(s3_client = regulations_bucket)
