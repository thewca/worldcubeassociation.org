...
class InboxResult < ApplicationRecord
  ...
  belongs_to :inbox_person, ->(ibr) { where(competitionId: ibr.competition_id) }, primary_key: :id, foreign_key: :person_id, optional: true
  ...
  def person
    InboxPerson.find_by(id: person_id, competitionId: competition_id)
  end
