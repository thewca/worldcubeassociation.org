  ...
  def payment_finish
    if current_user
      ...
      payment_intent.update_status_and_charges(stripe_intent, current_user) do |charge|
        ...
        begin
          Microservices::Registrations.update_registration_payment(attendee_id, charge.id, ruby_money.cents, ruby_money.currency.iso_code, stripe_intent.status, { type: "user", id: current_user.id })
