...
class SanityCheckCategoryJob < WcaCronjob
  def perform(options)
    ...
    SanityCheck.where(sanity_check_category_id: sanity_check_category_id).each do |sanity_check|
      ...
      SanityCheckResult.create!(sanity_check_id: sanity_check.id,
                                ...
                                cronjob_statistic_id: self.class.cronjob_statistics(options[:name]).id)
    end
  end
end
