...
class PublishRegistrationMetrics < WcaCronjob
  ...
  def record_next_60_minutes_bookmarks
    count = Competition.where('registration_open >= ? AND registration_open <= ?', Time.now, 60.minutes.from_now).sum(&:number_of_bookmarks)
  ...
  def record_next_60_minutes_competitor_limits
    total_limit = Competition.where('registration_open >= ? AND registration_open <= ?', Time.now, 60.minutes.from_now).sum(:competitor_limit)
