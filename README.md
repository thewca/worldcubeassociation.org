...
- Install docker-compose. The best way to get an up-to-date version is to use `pip install docker-compose` with Python 3 installed. On some systems, you may need to type `pip3` instead of `pip`.
- Navigate into the repository's main directory (`worldcubeassociation.org`)
- To start the server at `http://localhost:3000`, run `docker-compose up` and to bring it down, `docker-compose down`
