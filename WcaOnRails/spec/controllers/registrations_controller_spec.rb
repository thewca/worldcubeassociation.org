...
RSpec.describe RegistrationsController do
  context "signed in as organizer" do
    ...
    it "can delete multiple registrations" do
      ...
      expect do
        xhr :patch, :do_actions_for_slected, competition_id: competition.id, registrations_action: "delete-selected", selected_registrations: ["registration-#{registration.id}", "registration-#{registration2.id}"]
    ...
    it "can reject multiple registrations" do
      ...
      expect do
        xhr :patch, :do_actions_for_slected, competition_id: competition.id, registrations_action: "reject-selected", selected_registrations: ["registration-#{registration.id}", "registration-#{registration2.id}", "registration-#{pending_registration.id}"]
      end.to change { ActionMailer::Base.deliveries.length }.by(2)
      expect(registration.reload.pending?).to be true
    ...
    it "can accept multiple registrations" do
      ...
      expect do
        xhr :patch, :do_actions_for_slected, competition_id: competition.id, registrations_action: "accept-selected", selected_registrations: ["registration-#{registration.id}", "registration-#{registration2.id}", "registration-#{accepted_registration.id}"]
      end.to change { ActionMailer::Base.deliveries.length }.by(2)
      expect(registration.reload.accepted?).to be true
