...
RSpec.describe AdminController, type: :controller do
  ...
  describe 'anonymize_person' do
    ...
    it 'can anonymize person' do
      ...
      expect(response.status).to eq 200
      expect(response).to render_template :anonymize_person
      expect(flash.now[:success]).to eq "Successfully anonymized #{person.wca_id} to #{person.wca_id[0..3]}ANON01! Don't forget to run Compute Auxilery Data and Export Public."
    end
  end
