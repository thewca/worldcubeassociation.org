...
RSpec.describe AnonymizePerson do
  ...
  it "generates a wca id for ANON with the same year" do
    expect(anonymize_person.generate_new_wca_id).to eq true
    expect(anonymize_person.new_wca_id).to eq "2020ANON01"
  end
  ...
  it "can anonymize person and results" do
    ...
    expect(person.reload.year).to eq 1954
    expect(person.reload.month).to eq 12
    expect(person.reload.day).to eq 4
  end
  ...
  it "can anonymize account data" do
    ...
    expect(user.reload.dob.strftime("%Y-%m-%d")).to eq "1954-12-04"
