    ...
    if current_user
      ActiveRecord::Base.connected_to(role: :read_replica) do
        ...
        bookmarked_competitions = Competition.not_over
                                             ...
                                             .sort_by(&:start_date)
        render json: { past_competitions: past_competitions, future_competitions: not_past_competitions, registered_for_by_competition_id: registered_for_by_competition_id, bookmarked_competitions: bookmarked_competitions }
      end
    else
      render status: :unauthorized, json: { error: I18n.t('api.login_message') }
    end
